...
# OpenVINO backend isn't supported on macOS right now: Intel's
# OpenVINO.runtime.macos-arm64 NuGet stops at 2024.4.0.1, while the
# Python OpenVINO that produces IR is 2025.x+. The IR-format skew
# trips "Incorrect weights in bin file!" at `Core.read_model`. Tested
# Linux x86_64 and Windows where the version pair lines up.
case "$(uname -s)" in
    Darwin)
        echo "error: scripts/run-embed.sh is not supported on macOS." >&2
        echo "       OpenVINO native runtime NuGets max out at 2024.4 for" >&2
        echo "       macOS arm64, which can't read IR generated by current" >&2
        echo "       OpenVINO Python (2025.x+). Run this example on Linux" >&2
        echo "       (x86_64) or Windows where the versions align." >&2
        exit 2
        ;;
esac
